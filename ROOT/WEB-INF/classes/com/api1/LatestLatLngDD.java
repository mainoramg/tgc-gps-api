      ...
      if( splitURI != null && splitURI.length > 5 )
      {
         ...
         if( sn != null && !sn.equals( "" ) )
         {
            try
            {
               ...
               Vector            records     =  DB.getData( "select latitude_cardinal, latitude_degree, latitude_minutes, latitude_seconds, longitude_cardinal, longitude_degree, longitude_minutes, longitude_seconds, direction, DATE_FORMAT(CONVERT_TZ(location_stamp,'+00:00','-06:00'), '%M %e, %Y-%r') as locat_stamp from gps_data where serial_number = '" + sn + "' order by location_stamp desc limit " + limit );
               //Vector            records     =  DB.getData( "select latitude_cardinal, latitude_degree, latitude_minutes, latitude_seconds, longitude_cardinal, longitude_degree, longitude_minutes, longitude_seconds, direction, DATE_FORMAT(CONVERT_TZ(location_stamp,'+00:00','-06:00'), '%M %e, %Y-%r') as locat_stamp from gps_data_test order by stamp desc limit " + limit );
               ...
               for( int i = 0; i < total; i++ )
               {
                  ...
                  LocationLatLngDD  location =  new LocationLatLngDD(
                                                ...
                                                record.get("locat_stamp").toString()
                                             );
